...
import { budgetsApi, connectionsApi, detailMessage, jobsApi, operationsApi, pagesApi, policyApi, settingsApi, sitesApi, teamApi } from '../lib/api'
...
export function PoliciesPage() {
  ...
  const { role } = useAuth()
  const canEdit = role === 'owner'
  const loader = useCallback(async () => { const [policy, settings, site, connections, pages] = await Promise.all([policyApi.get(siteId), settingsApi.get(), sitesApi.get(siteId), connectionsApi.list(siteId), pagesApi.list(siteId, { limit: 200 })]); return { policy, settings, site, connections, pages } }, [siteId])
  const resource = useResource(loader, [siteId])
  ...
  const verifiedAuthors = useMemo(() => {
    ...
    const wordpress = resource.data?.connections.items.find((connection) => connection.kind === 'wordpress')
    ...
    for (const page of resource.data?.pages.items.filter((item) => item.resource_type === 'authors') ?? []) {
  ...
  const authorIsVerified = !settings.author_id || verifiedAuthors.some((author) => author.id === settings.author_id)
  ...
  async function save() {
    ...
    if (!authorIsVerified) {
      setError('The selected publishing author is not verified by the current WordPress connection. Test the connection or clear the author before saving.')
      return
    }
